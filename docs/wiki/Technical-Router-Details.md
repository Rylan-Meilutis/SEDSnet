...
- Topology changes publish an address/ownership summary and a node delta. Additions and changes
  upsert only their named nodes, removals carry named tombstones, and the idempotent delta is sent
  three times with exponential discovery backoff. An infrequent full snapshot remains as repair
  without treating every change as a blank-slate discovery cycle.
