...
fn reliable_control_survives_a_missing_compact_dictionary() {
    ...
    let status = crate::config::register_data_type_with_description(
        "ROUTER_RELIABLE_HEADER_RECOVERY",
...
fn check_restarting_peer_header(schema_request: bool) {
    ...
    let ty = crate::DataType::try_named("ROUTER_RESTART_BEST_EFFORT").unwrap_or_else(|| {
        crate::config::register_data_type_with_description(
            "ROUTER_RESTART_BEST_EFFORT",
            "",
            crate::MessageElement::Static(
                12,
                crate::MessageDataType::UInt8,
    ...
    let tx = Router::new_with_clock(RouterConfig::new([]).with_sender("GB"), Box::new(|| 100u64));
    let opts = RouterSideOptions::default().with_small_packet_transport(0);
    ...
    tx.rx_from_side(
        &discovery::build_discovery_announce("GS", 1, &[endpoint]).unwrap(),
        side,
    )
    .unwrap();
    ...
    assert_eq!(
        tx.state.lock().side_transport[&side].rx_template_count(),
        1,
    ...
    assert!(
        empty
            .decode_side_transport_frame(empty_side, &unrelated_frames[0])
            .unwrap()
            .is_none(),
...
fn reliable_missing_route_is_an_error_and_same_packet_can_retry_after_discovery() {
    ...
    router
        .rx_from_side(
            &discovery::build_discovery_announce("PEER", 1, &[DataEndpoint::named("SD_CARD")])
                .unwrap(),
            side,
        )
        .unwrap();
    let packet = Packet::new(ty, &[destination], "VB", 42, Arc::from([1u8].as_slice())).unwrap();
    assert!(
        router.tx(packet.clone()).is_err(),
        "a missing remote route must not masquerade as successful delivery"
    );
    assert_eq!(received.load(Ordering::Relaxed), 0);
    router
        .rx_from_side(
            &discovery::build_discovery_announce("PEER", 2, &[destination]).unwrap(),
            side,
        )
        .unwrap();
...
fn discovery_refresh_survives_a_missed_transport_template() {
    ...
    for timestamp in [1000, 6000] {
        ...
        assert!(
            decoded.is_some(),
            "discovery must bootstrap without a dictionary"
        );
    }
}
